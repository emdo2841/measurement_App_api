...
const refreshToken = async (req, res) => {
    ...
    try {
        ...
        const decoded = jsonwebtoken_1.default.verify(incomingToken, REFRESH_SECRET);
        const hashed = (0, auth_1.hashToken)(incomingToken);
        // 2. Find token in DB
        ...
        const newAccessToken = (0, auth_1.generateAccessToken)(user.id, user.email);
        const newRefreshToken = (0, auth_1.generateRefreshToken)(user.id);
        // 6. Save NEW Refresh Token to DB
        const newHashed = (0, auth_1.hashToken)(newRefreshToken);
...
const forgotPassword = async (req, res) => {
    ...
    const { rawToken, hashedToken, expiresAt } = (0, cryptos_1.generateResetToken)(15);
    ...
    const htmlContent = (0, emailTemplate_1.passwordResetTemplate)(user.name, resetUrl);
    try {
...
const resetPassword = async (req, res) => {
    ...
    const hashedToken = (0, auth_1.hashToken)(token);
